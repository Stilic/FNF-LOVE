...
function Slider:new(x, y, width, height, value, sliderType, min, max)
    ...
    if self.sliderType == "horizontal" then
        local clampedX = math.max(self.min, math.min(self.width - self.height, self.x))
        self.value = self.min + (clampedX / (self.width - self.height)) * (self.max - self.min)
...
function Slider:update()
    if self.isDragging then
        if self.sliderType == "horizontal" then
            ...
            local clampedX = math.max(self.min, math.min(self.width - self.height, relativeX))
            self.value = self.min + (clampedX / (self.width - self.height)) * (self.max - self.min)
        elseif self.sliderType == "vertical" then
...
function Slider:__render(camera)
    ...
    if self.sliderType == "horizontal" then
        ...
        local knobX = self.x + (self.value - self.min) / (self.max - self.min) * (self.width - self.height)
        local knobY = self.y + self.height / 2
        love.graphics.setColor(self.knobColor)
        love.graphics.rectangle("fill", knobX, knobY - self.height / 2, self.height, self.height)
    elseif self.sliderType == "vertical" then
        love.graphics.rectangle("fill", self.x, self.y, self.width, self.height)
        local knobX = self.x + self.width / 2
        local knobY = self.y + (self.value - self.min) / (self.max - self.min) * (self.height - self.width)
        love.graphics.setColor(self.knobColor)
        love.graphics.rectangle("fill", knobX - self.width / 2, knobY, self.width, self.width)
    end
end
