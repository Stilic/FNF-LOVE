local source = (...) and (...):gsub('%.ui.window$', '.') or ""
local Object = require(source .. "object")
local Group = require(source .. "group.group")
local Color = require(source .. "util.color")
local UIButton = require(source .. "ui.button")
...
function Window:new(x, y, width, height, title)
    ...
    self.exitButton = UIButton(0, 0, 22, 22, 'X', function()
        self:kill()
        if self.onClosed then self.onClosed() end
    end)
    self.exitButton.color = Color.RED
    self.minButton = UIButton(0, 0, 22, 22, '-', function()
