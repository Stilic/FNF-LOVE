local source = (...) and (...):gsub('%.oldui.numericstepper$', '.') or ""
...
function NumericStepper:new(x, y, stepSize, defaultValue, min, max)
	...
	-- button stepper
	self.onChanged = nil
	self.increaseButton = UIButton(0, 0, self.height, self.height, "+",
		function()
	...
	self.decreaseButton = UIButton(0, 0, self.height, self.height, "-",
		function()
			local num = tonumber(self.value)
			local result = num - self.stepSize
