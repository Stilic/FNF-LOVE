...
function PlayState:enter()
	...
	PlayState.song = {
		...
		girlfriend = chart.gfVersion == nil and
			(chart.player3 == nil and "ralt-gf" or chart.player3) or chart.gfVersion,
		mustHits = {}
	}
...
function PlayState:update(dt)
	...
	if mustHit ~= nil then
		local camPosX, camPosY, midpoint = 0, 0
		if mustHit then
			midpoint = self.boyfriend:getMidpoint()
			camPosX = self.boyfriend.cameraPosition.x + self.stage.boyfriendCam.x
			camPosY = self.boyfriend.cameraPosition.y + self.stage.boyfriendCam.y
		else
			midpoint = self.dad:getMidpoint()
			camPosX = self.dad.cameraPosition.x + self.stage.dadCam.x
			camPosY = self.dad.cameraPosition.y + self.stage.dadCam.y
		end
		self.camFollow.x, self.camFollow.y = midpoint.x + 150 + camPosX, midpoint.y - 100 + camPosY
	end
	...
	for i, n in ipairs(self.allNotes.members) do
		if n.mustPress then
			if n.isSustain and self.keysPressed[n.data] and n.parentNote and
				not n.parentNote.hasMissed and n.parentNote.wasGoodHit and
				n.canBeHit then self:goodNoteHit(n) end
...
function sortByShit(a, b) return a.time < b.time end
	...
	for _, n in ipairs(self.allNotes.members) do
		if n.mustPress and not n.tooLate then
			if not n.isSustain and not n.wasGoodHit then
				...
				if n.canBeHit then
					if n.data == key then table.insert(noteList, n) end
					canMiss = true
				end
			elseif n.isSustain and  n.data == key and ((n.wasGoodHit or n.prevNote.wasGoodHit) and
				(n.parentNote ~= nil and not n.parentNote.hasMissed and n.parentNote.wasGoodHit))
			then
...
function PlayState:goodNoteHit(n)
	if not n.wasGoodHit then
		...
		char.holding = n.mustPress or (n.isSustain and not n.isSustainEnd)
