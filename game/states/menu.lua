...
function MainMenuState:enter()
    ...
    Camera.defaultCamera = self.camScroll
    ...
    for i = 0, #self.optionShit - 1 do
        ...
        menuItem:setFrames(paths.getSparrowAtlas('menus/mainmenu/menuoptions/menu_'..self.optionShit[i + 1]))
        menuItem:addAnimByPrefix('idle', self.optionShit[i + 1]..' basic', 24)
        menuItem:addAnimByPrefix('selected', self.optionShit[i + 1]..' white', 24)
        menuItem:play('idle')
        menuItem.ID = (i + 1)
    ...
    self.daText = Text(12, push:getHeight() - 24, "FNF-LOVE v0.5", paths.getFont("vcr.ttf", 16), {255, 255, 255})
...
function MainMenuState:update(dt)
    ...
    self.camScroll.target.x, self.camScroll.target.y =
        util.coolLerp(self.camScroll.target.x, self.camFollow.x, 1),
        util.coolLerp(self.camScroll.target.y, self.camFollow.y, 1)
                ...
                for i, spr in pairs(self.menuItems.members) do
                    if MainMenuState.curSelected ~= spr.ID then
                        ...
                        Flicker(spr, 1, 0.06, false, false, function()
                            local daChoice = self.optionShit[MainMenuState.curSelected]
...
function MainMenuState:changeSelection(huh)
    ...
    if MainMenuState.curSelected > #self.optionShit then MainMenuState.curSelected = 1
    elseif MainMenuState.curSelected < 1 then MainMenuState.curSelected = #self.optionShit end
    ...
    for _, spr in pairs(self.menuItems.members) do
        ...
        if spr.ID == MainMenuState.curSelected then
            spr:play('selected')
            local add = 0
            if #self.menuItems > 4 then
                add = #self.menuItems * 8
            end
            self.camFollow = {x = spr:getGraphicMidpoint().x, y = spr:getGraphicMidpoint().y - add}
            spr:centerOffsets()
        end
    end
end
