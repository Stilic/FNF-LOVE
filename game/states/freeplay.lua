...
function FreeplayState:enter()
    ...
    for i = 0, #self.songs - 1 do
        local songText = Alphabet(0, (70 * i) + 30, self.songs[i+1], true, false)
...
function FreeplayState:update(dt)
    ...
    if controls:pressed('accept') then
        PlayState.curSong = self.songs[FreeplayState.curSelected]
...
function FreeplayState:changeSelection(huh)
    ...
    if FreeplayState.curSelected > #self.songs then FreeplayState.curSelected = 1
    elseif FreeplayState.curSelected < 1 then FreeplayState.curSelected = #self.songs end
