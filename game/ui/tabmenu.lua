...
local function tranformChildren(self, func, value)
    for i, grp in ipairs(self.group) do if grp.name == self.tabs[self.selectedTab] then
...
function TabMenu:update(dt)
    if Mouse.justPressed then
        if Mouse.justPressedLeft then self:mousepressed(Mouse.x, Mouse.y, 1)
        elseif Mouse.justPressedRight then self:mousepressed(Mouse.x, Mouse.y, 2)
        elseif Mouse.justPressedMiddle then self:mousepressed(Mouse.x, Mouse.y, 3) end
    end
    if Mouse.justReleased then
        if Mouse.justReleasedLeft then self:mousereleased(Mouse.x, Mouse.y, 1)
        elseif Mouse.justReleasedRight then self:mousereleased(Mouse.x, Mouse.y, 2)
        elseif Mouse.justReleasedMiddle then self:mousereleased(Mouse.x, Mouse.y, 3) end
    end
    ...
    for _, cam in ipairs(cameras) do
        ...
        if self.tabs then
            for i, tab in ipairs(self.tabs) do
                ...
                local tabY = self.y
                local tabColor = (i == self.selectedTab) and {0.5, 0.5, 0.5} or {0.3, 0.3, 0.3}

                love.graphics.setColor(tabColor)
                love.graphics.rectangle("fill", tabX, tabY, (self.width/#self.tabs), self.tabHeight)

                love.graphics.setColor(0.3, 0.3, 0.3)
                love.graphics.rectangle("line", tabX, tabY, (self.width/#self.tabs), self.tabHeight)

                love.graphics.setColor(1, 1, 1)
                love.graphics.setFont(self.font)
                local textWidth = self.font:getWidth(tab)
                local textHeight = self.font:getHeight(tab)
                love.graphics.print(tab, tabX + ((self.width/#self.tabs) - textWidth) / 2, tabY + (self.tabHeight - textHeight) / 2)
            end
        end

        love.graphics.setColor(0.5, 0.5, 0.5)
        love.graphics.rectangle("fill", self.x, self.y + self.tabHeight, self.width + (#self.tabs + 3), self.height)

        love.graphics.setColor(0.3, 0.3, 0.3)
        love.graphics.rectangle("line", self.x, self.y + self.tabHeight, self.width + (#self.tabs + 3), self.height)
...
local function isMouseOverTab(self, mx, my)
    if self.tabs then
        for i, _ in ipairs(self.tabs) do
            local tabX = self.x + (i - 1) * ((self.width/#self.tabs) + 2)
            local tabY = self.y
            if mx >= tabX and mx <= tabX + (self.width/#self.tabs) and my >= tabY and my <= tabY + self.tabHeight then
                return i
            end
        end
    end
...
function TabMenu:mousereleased(x, y, button, istouch, presses)
    if self.tabs then callChildren(self, 'mousereleased', x, y, button, istouch, presses) end
end


function TabMenu:keypressed(key, scancode, isrepeat)
    if self.tabs then callChildren(self, 'keypressed', key, scancode, isrepeat) end
end
