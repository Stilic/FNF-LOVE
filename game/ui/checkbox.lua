...
function Checkbox:update(dt)
    ...
    self.hovered = (mx >= self.x and mx <= self.x + self.size and
...
function Checkbox:draw()
    ...
    for _, cam in ipairs(cameras) do
        ...
        if self.checked then
            ...
            love.graphics.rectangle("fill", logo_position[1], logo_position[2], (self.size * 0.5), (self.size * 0.2))
            love.graphics.rectangle("fill", 0, logo_position[2] * -2, (self.size * 0.2), (self.size * 1.1))
