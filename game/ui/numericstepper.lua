...
function NumericStepper:new(x, y, stepSize, defaultValue, min, max)
    ...
    self.value = type(defaultValue) == "number" and tostring(defaultValue) or "1"
    ...
    -- button stepper
    self.onChanged = nil
    self.increaseButton = ui.UIButton(0, 0, self.height, self.height, "+", function()
    ...
    self.decreaseButton = ui.UIButton(0, 0, self.height, self.height, "-", function()
        local num = tonumber(self.value)
        local result = num - self.stepSize
...
function NumericStepper:draw()
    ...
    for _, cam in ipairs(cameras) do
        ...
        love.graphics.print(self.value, -self.__scrollTextX, (self.height - self.font:getHeight()) / 2)
        love.graphics.pop()
        love.graphics.setScissor()

        if self.active and true then
            love.graphics.setColor(self.colorCursor)
            love.graphics.rectangle("fill", self.x - self.__scrollTextX + 5 + self.font:getWidth(self.value:sub(1, self.__cursorPos)), self.y + 3, 1, self.height - 6)
        end
        ...
        self.decreaseButton.x = self.increaseButton.x + (self.increaseButton.width * 1.05)
...
function NumericStepper:update(dt)
    if self.active then
        ...
        if self.value == "" then self.value = self.defaultVal
        elseif tonumber(self.value) > self.max then self.value = tostring(self.max)
        elseif tonumber(self.value) < self.min then self.value = tostring(self.min) end

        if self.__input and self.__typing then
            self.__typing = false
            local newText = self.value:sub(1, self.__cursorPos) .. self.__input .. self.value:sub(self.__cursorPos + 1)
            self.value = newText
            self.__cursorPos = self.__cursorPos + utf8.len(self.__input)
            self.__input = ""
            if self.onChanged then self.onChanged(tonumber(self.value)) end
        end
        ...
        if self.__cursorMove then
            ...
            if self.__cursorMoveTimer >= self.__cursorBlinkTime then
                if self.__cursorMoveDir == CursorDirection.LEFT and self.__cursorPos > 0 then
                    self.__cursorPos = self.__cursorPos - 1
                elseif self.__cursorMoveDir == CursorDirection.RIGHT and self.__cursorPos < utf8.len(self.value) then
                    self.__cursorPos = self.__cursorPos + 1
                end
        ...
        if self.__removePressed then
            ...
            if self.__removeTimer >= self.__removeTime then
                if self.__removeType == RemoveType.BACKSPACE and self.__cursorPos > 0 then
                    local byteoffset = utf8.offset(self.value, -1, self.__cursorPos + 1)
                    if byteoffset then
                        self.value = string.sub(self.value, 1, byteoffset - 1) .. string.sub(self.value, byteoffset + 1)
                        self.__cursorPos = self.__cursorPos - 1
                    end
                elseif self.__removeType == RemoveType.DELETE and self.__cursorPos < utf8.len(self.value) then
                    local byteoffset = utf8.offset(self.value, 1, self.__cursorPos + 1)
                    if byteoffset then
                        self.value = string.sub(self.value, 1, byteoffset - 1) .. string.sub(self.value, byteoffset + 1)
                    end
            ...
            if self.__newTextWidth > self.__prevTextWidth then
                self.__scrollTextX = math.max(self.__scrollTextX - (self.__newTextWidth - self.__prevTextWidth), 0)
            elseif self.__newTextWidth < self.__prevTextWidth and self.__scrollTextX > 0 then
                self.__scrollTextX = math.min(self.__scrollTextX + (self.__prevTextWidth - self.__newTextWidth), self.__prevTextWidth - self.canvas:getWidth())
            end
        end
        ...
        local cursorX = self.x + 5 + self.font:getWidth(self.value:sub(1, self.__cursorPos)) - self.__scrollTextX
        if cursorX > self.x + self.width - 10 then
            self.__scrollTextX = self.__scrollTextX + (cursorX - (self.x + self.width - 10))
