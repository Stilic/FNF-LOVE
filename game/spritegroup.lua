...
local function cameraTransform(sprite, cameras) sprite.cameras = cameras end
...
function SpriteGroup:update(dt)
    if self.__camera ~= self.cameras then
        tranformChildren(self, cameraTransform, self.cameras)
        self.__camera = self.cameras
    end
