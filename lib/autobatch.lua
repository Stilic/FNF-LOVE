...
local function switchActiveImage(img)
    ...
    if img then
        ...
        if not b then
            b = {}
            b.sb = love_graphics.newSpriteBatch(img, nil, "stream")
...
function autobatch.draw(image, ...)
    ...
    -- Add to spritebatch
    autobatch.batches[autobatch.image].sb:add(...)
end
...
function autobatch.setColor(r, g, b, a)
    ...
    if t[1] == r and t[2] == g and t[3] == b and t[4] == a then return end
    -- Set active color
...
function autobatch.getColor() return unpack(autobatch.color) end
...
function autobatch.pop()
    ...
    if color ~= false then autobatch.setColor(color) end
end
...
local names = {
    "arc", "circle", "clear", "discard", "ellipse", "line", "points", "polygon",
    "present", "print", "printf", "rectangle", "stencil", "getStats",
    "setBlendMode", "setCanvas", "setColorMask", "setScissor", "setShader",
    "setStencilTest", "origin", "rotate", "scale", "shear", "translate"
}

for i, name in ipairs(names) do autobatch[name] = wrap(love_graphics[name]) end
