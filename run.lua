...
local consolas, real_fps = love.graphics
                               .newFont('assets/fonts/consolas.ttf', 14), 0
function love.run()
    ...
    local function draw()
        ...
        if love.showFPS then
            _stats, _update = love.graphics.getStats(),
                              love.timer.getUpdateFPS()
            _fps = math.min(love.parallelUpdate and real_fps or _update,
                            love.FPScap)
            _ram, _vram = math.countbytes(collectgarbage("count") * 0x400),
                          math.countbytes(_stats.texturememory)
            _text = love.parallelUpdate and
                        fpsParallelFormat:format(_fps, _update, _ram, _vram,
                                                 _stats.drawcalls) or
    ...
    local fpsUpdateFrequency, prevFpsUpdate, timeSinceLastFps, frames = 1, 0, 0,
                                                                        0
    ...
    return function()
            ...
            if love.graphics.isActive() then
                if love.parallelUpdate then
                    ...
                    if clock + real_dt > nextclock then
                        draw()
                        nextclock = cap + clock
                        timeSinceLastFps, frames = clock - prevFpsUpdate,
                                                   frames + 1
                        if timeSinceLastFps > fpsUpdateFrequency then
                            real_fps, frames =
                                math.round(frames / timeSinceLastFps), 0
                            prevFpsUpdate = clock
                        end
                    end
                else
        ...
        if not love.parallelUpdate or not focused then
            ...
            if real_dt < 0.001 then love.timer.sleep(0.001) end
        end
        firstTime = false
    end
end
...
function love.errorhandler(msg)
    ...
    function firstPass()
        ...
        fnfFont18 = paths.getFont("phantommuff.ttf", 18) or
                        love.graphics.setNewFont(18)
        fnfFont20 = paths.getFont("phantommuff.ttf", 35) or
                        love.graphics.setNewFont(35)

        bgMusic = paths.getMusic("pause/railways", "static")
        missSfx = love.audio.newSource(paths.getSound(
                                           "gameplay/missnote" ..
                                               love.math.random(1, 3)), "static")
    ...
    local function draw(force)
        ...
        scale2 = math.max(math.min(gameW, 1600) / funkinLogoW,
                          math.min(gameH, 900) / funkinLogoH) * 0.7

        love.graphics.setColor(0.2, 0.2, 0.2)
        love.graphics.draw(menuDesat, hgameW, hgameH, 0, scale1, scale1,
                           menuDesatW / 2, menuDesatH / 2)
        love.graphics.draw(funkinLogo, hgameW, hgameH, 0, scale2, scale2,
                           funkinLogoW / 2, funkinLogoH / 2)

        love.graphics.setColor(1, 1, 1)

        love.graphics.setFont(fnfFont20)
        love.graphics.printf(__error__, 40, 40, love.graphics.getWidth() - 80,
                             __center__)

        love.graphics.setFont(fnfFont18)
        love.graphics.printf(p, 40, 110, love.graphics.getWidth() - 80,
                             __center__)
    ...
    local eventhandlers = {
        ...
        focus = function(f) bgMusic:setVolume(f and 0.3 or 0.1) end,
        resize = function(w, h)
            gameW, gameH = w, h
            draw()
        end,
    ...
    if love.system.getDevice() == "Mobile" then
        ...
        return function()
            ...
            if not done then firstPass() end
